...
\subsubsection{Dataset detail.} We construct a synthetic spatial relation dataset from ImageNet crops by compositing two objects on a shared canvas and assigning one of four diagonal relations: \texttt{top-left}, \texttt{top-right}, \texttt{bottom-left}, and \texttt{bottom-right}. The same image is reused for both understanding and generation. For understanding, each image is paired with a natural-language question and a full-sentence answer. The question explicitly specifies the required response format so that the target output is a sentence rather than a bare relation label. An example is shown below.

\begin{quote}
\textbf{Question:} With respect to the digital clock, where can you find the flat-coated retriever? Response format: The flat-coated retriever can be found to the top-left/top-right/bottom-left/bottom-right of the digital clock. \\
\textbf{Answer:} The flat-coated retriever can be found to the top-right of the digital clock.
\end{quote}

For generation, we reuse the target answer sentence itself as the text prompt. As a result, generation prompts take the form of declarative relation statements, \textit{e.g.}, ``The flat-coated retriever can be found to the top-right of the digital clock.''
\input{x_supple/tabs/dataset_spatial}
\subsubsection{Understanding evaluation.} We evaluate spatial-relation understanding on the first 100 samples from the validation split of the same dataset. The model receives the original question string, including the response-format clause shown above, and generates a free-form answer sentence. We mark a prediction as correct by parsing a single diagonal relation word from both the prediction and the ground-truth answer. Predictions that contain no valid relation word or mention multiple relations are counted as incorrect.
\subsubsection{Generation evaluation.} 
